...
test("test_empty_string2", () => {
  const text_styler = new TextStyler<string>([
    new TextStylerConfig("*", htmlTag("strong")), // unused
  ]);
  expect(text_styler.processText("").join("")).toBe("");
});
...
test("test_all_is_bold", () => {
  const text_styler = new TextStyler<string>([new TextStylerConfig("*", htmlTag("strong"))]);
  expect(text_styler.processText("*this is bold*").join("")).toBe("<strong>this is bold</strong>");
});

test("test_all_is_italics", () => {
  const text_styler = new TextStyler<string>([new TextStylerConfig("_", htmlTag("em"))]);
...
test("test_empty_bold3", () => {
  const text_styler = new TextStyler<string>([new TextStylerConfig("*", htmlTag("strong", undefined, false))]);
  expect(text_styler.processText("**").join("")).toBe("<strong></strong>");
});

test("test_empty_bold4", () => {
  const text_styler = new TextStyler<string>([
    new TextStylerConfig("*", htmlTag("strong", undefined, true), {
...
test("test_italics_within_bold", () => {
  const text_styler = new TextStyler<string>([
    new TextStylerConfig("*", htmlTag("strong")),
    new TextStylerConfig("_", htmlTag("em")),
  ]);
...
test("test_strikethrough", () => {
  const text_styler = new TextStyler<string>([
    new TextStylerConfig("~~", htmlTag("del")),
    new TextStylerConfig("*", htmlTag("strong")),
  ]);
...
test("test_strikethrough_and_sub", () => {
  const text_styler = new TextStyler<string>([
    new TextStylerConfig("~", htmlTag("sub")),
    new TextStylerConfig("~~", htmlTag("del")),
  ]);
...
test("test_different_start_and_end", () => {
  const text_styler = new TextStyler<string>([
    new TextStylerConfig("<!", htmlTag("span", { class: "spoiler" }), { end: "!>" }),
  ]);
...
test("test_start_and_end_inside_start_and_end_disallow_direct", () => {
  const text_styler = new TextStyler<string>([
    new TextStylerConfig("_", htmlTag("em")),
    new TextStylerConfig("<!", htmlTag("span", { class: "spoiler" }), {
      end: "!>",
...
test("test_multiquote", () => {
  const text_styler = new TextStyler<string>([new TextStylerConfig(">", htmlTag("blockquote"), { end: "\n" })]);
  expect(
...
test("test_multiquote_preserving_start_marking", () => {
  const text_styler = new TextStyler<string>([
    new TextStylerConfig(">", htmlTag("blockquote"), {
      end: "\n",
      consume_start: ConsumptionStyle.OUTSIDE,
    }),
  ]);
...
test("test_non_html_transform", () => {
  const text_styler = new TextStyler<string>([
    new TextStylerConfig("!", (children: string[]) => children.join("").toUpperCase(), { end: "!" }),
  ]);
...
test("test_regex_within_regex", () => {
  const text_styler = new TextStyler<string>([
    new TextStylerRegexConfig(/\/anime\//, (match: RegExpMatchArray) => `<link to='/anime'>/anime/</link>`),
    new TextStylerRegexConfig(
      /https:\/\/\w+\.\w+\.com(\/\w+)+/,
